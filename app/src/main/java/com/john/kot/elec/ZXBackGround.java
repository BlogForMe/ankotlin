...
public class ZXBackGround extends View {
    ...
    public ZXBackGround(Context context) {
        super(context);
        initScreen(context);
    }

    public ZXBackGround(Context context, AttributeSet attrs) {
        super(context, attrs);
        initScreen(context);
    }

    public ZXBackGround(Context context, AttributeSet attrs, int defStyleAttr) {
...
//		mPaint.setColor(Color.rgb(0xba, 0xba, 0xba));
        ...
        int i = gridCnt / 2;
        for (int j = 0; j < i; j++) {
            canvas.drawLine(0, fMMgetPxfory(fPXgetMMforY(mHeight / 2) - gridHeigh * j)
                    , mWidth,
...
//		Log.i("fMMgetPxfory	",  " mm" + mm + " ");
    ...
    public void setGridHeigh(float gridHeigh) {
        ZXBackGround.gridHeigh = gridHeigh;
    }
