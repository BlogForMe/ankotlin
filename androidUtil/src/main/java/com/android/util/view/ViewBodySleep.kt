...
class ViewBodySleep : View {
    ...
    private val indexRect: Rect

    val wakeHeight = dp2px(context, 67f) //清醒高度
    val paleHeight = dp2px(context, 100f) //浅度睡眠
    val darkHeight = dp2px(context, 130f) //深度睡眠
    var mWidth = 0f
    private val linePaint: Paint = Paint().apply { //横线和 刻度颜色
        color = Color.parseColor("#DDDDDD");
        strokeWidth = dp2px(context, 2f)
        textSize = sp2px(context, 21f)
    }
...
//        Timber.i("width $w height $h")
//        Timber.i("paddingLeft $paddingLeft  paddingRight $paddingRight")
...
//        Timber.i("unitWidth $unitWidth")
        ...
        val TUBE_TOP =  SLEEP_HEIGHT + dp2px(context,20f) //管道上边坐标
        val TUBE_DIAMETER = dp2px(context,40f) //圆直径
        val UNINT_TUBE =(mWidth-TUBE_DIAMETER)/72 //管道的单位长度
        ...
        darkPath.addArc(rectF,90f,180f)
        darkPath.lineTo(paddingLeft+TUBE_DIAMETER/2f+UNINT_TUBE*darkCount,TUBE_TOP)
        darkPath.lineTo(paddingLeft+TUBE_DIAMETER/2f+UNINT_TUBE*darkCount,TUBE_TOP+TUBE_DIAMETER)
        rectPaint.color = Color.parseColor("#3989AC")
        ...
        rectPaint.color = Color.parseColor("#D1ECEE")
        canvas?.drawRect(paddingLeft+TUBE_DIAMETER/2f+UNINT_TUBE*(darkCount+paleCount),TUBE_TOP,mWidth+TUBE_DIAMETER/2,TUBE_TOP+TUBE_DIAMETER,rectPaint)
        val wakePath = Path()
        val rectWake = RectF(mWidth,TUBE_TOP,mWidth+TUBE_DIAMETER,TUBE_TOP+TUBE_DIAMETER)
