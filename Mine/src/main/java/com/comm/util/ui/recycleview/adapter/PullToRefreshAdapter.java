...
public class PullToRefreshAdapter extends BaseQuickAdapter<Status, BaseViewHolder> {
    ClickableSpan clickableSpan = new ClickableSpan() {
        ...
        public void updateDrawState(TextPaint ds) {
            //            ds.setColor(Utils.getContext().getResources().getColor(R.color
            //            .clickspan_color));
    ...
    protected void convert(BaseViewHolder helper, Status item) {
        ...
        String msg
            = "\"He was one of Australia's most of distinguished artistes, renowned for his "
            + "portraits\"";
        String mdata = item.getUserName() + "     " + item.getCreatedAt();
        ((TextView)helper.getView(R.id.tweetText)).setText(SpannableStringUtils.getBuilder(mdata)
            .append("landscapes and nedes")/*.setClickSpan(clickableSpan)*/.create());
        ((TextView)helper.getView(R.id.tweetText)).setMovementMethod(
