...
public class SpanTag extends HtmlTag {
    ...
    private static void setSpanFromMark(Spannable text, Object mark, Object... spans) {
        ...
        if (where != len) {
            for (Object span : spans) {
                text.setSpan(span, where, len,
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);  //注意：开始位置和结束位置，因为SpannableStringBuilder
                // 的append添加字符方法导致len已经大于where了
            }
        }
    }
    ...
    public void endHandleTag(Editable text) {
        ...
        if (b != null) {
            end(text, Background.class, new BackgroundColorSpan(
                b.color)); //设置为Android可以解析的24种ParcelableSpan基本分类，当然也可以自己定义，但需要集成原有的分类
        }
        ...
        if (f != null) {
                ...
                new TextFontSpan(f.textSize, f.textDecordation, f.fontWeidght)); //使用自定义的
        }
    }
