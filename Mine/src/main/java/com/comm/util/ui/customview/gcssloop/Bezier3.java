...
public class Bezier3 extends View {
    ...
    private final PointF mCneter = new PointF(0, 0);
    private final float mCircleRadius = 200;                  // 圆的半径
    private final float mDifference = mCircleRadius * C;      // 圆形的控制点与数据点的差值
    private final float[] mData = new float[8];                // 顺时针记录绘制圆形的四个数据点
    private final float[] mCtrl = new float[16];              // 顺时针记录绘制圆形的八个控制点
    private final float mDuration = 1000;                  // 变化总时长
    private final float mCount = 100;         // 将时长总共划分多少份
    private final float mPiece = mDuration / mCount;  // 每一份的时长
