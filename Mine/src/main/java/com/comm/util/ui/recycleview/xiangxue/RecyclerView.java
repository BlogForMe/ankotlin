    ...
    public void scrollBy(int x, int diff) {
        ...
        if (scrollY > 0) {
            ...
            while (heights[firstRow] < scrollY) {
                if (!viewList.isEmpty()) {
                    //移除最顶上的View
                    Log.e(TAG, "-----移除-----" + ((TextView)viewList.get(0).findViewById(
                        R.id.text1)).getText());
            ...
            while (getFilledHeight() < height) {
                ...
                Log.i(TAG, "添加dataIndex：" + dataIndex + "getChildCount()"
            ...
            while (!viewList.isEmpty() && getFilledHeight() - height > heights[
                firstRow + viewList.size() - 1]) {
        ...
        if (scrollY == 0) {
            ...
            */
            scrollY = Math.min(scrollY,
                Math.max(0, sumArray(firstRow, adapter.getCount() - firstRow) - height));
