 ...
 * @author huangziwei
 * @date 2016.1.11
 * https://blog.csdn.net/u012964944/article/details/70172885
 */
public abstract class ScrollPickerView<T> extends View {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            if (typedArray.hasValue(R.styleable.ScrollPickerView_spv_center_item_background)) {
                setCenterItemBackground(typedArray.getDrawable(
            ...
            setCenterPosition(typedArray.getInt(
                ...
                getCenterPosition()));
            setIsCirculation(typedArray.getBoolean(R.styleable.ScrollPickerView_spv_is_circulation,
                isIsCirculation()));
            setDisallowInterceptTouch(
                typedArray.getBoolean(R.styleable.ScrollPickerView_spv_disallow_intercept_touch,
                    isDisallowInterceptTouch()));
            setHorizontal(typedArray.getInt(R.styleable.ScrollPickerView_spv_orientation,
                mIsHorizontal ? 1 : 2) == 1);
            typedArray.recycle();
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = start; i >= 1; i--) { // 先从远离中间位置的item绘制，当item内容偏大时，较近的item覆盖在较远的上面
            ...
            if (mDrawAllItem || i <= mCenterPosition + 1) {  // 上面的items,相对位置为 -i
                ...
                // 传入位置信息，绘制item
                if (mIsCirculation) {
                    drawItem(canvas, mData, position, -i, mMoveLength,
     ...
     * @param relative   相对中间item的位置,relative==0表示中间item,relative<0表示上（左）边的item,relative>0表示下(右)
     *                   边的item
     ...
     */
    public abstract void drawItem(Canvas canvas, List<T> data, int position, int relative,
                                  float moveLength, float top);
    ...
    private void reset() {
        ...
        if (mCenterItemBackground != null) {
            mCenterItemBackground.setBounds(mCenterX, mCenterY, mCenterX + mItemWidth,
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            ...
            if (mCanTap && !mIsScrollingLastTime) {
                if (lastMove >= mCenterPoint
                    && lastMove <= mCenterPoint + mItemSize) { //点击中间item，回调点击事件
                    performClick();
