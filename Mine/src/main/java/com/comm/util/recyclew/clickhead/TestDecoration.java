...
public class TestDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int layoutPos = 0; layoutPos < count; layoutPos++) {
            ...
            if (adapterPos != RecyclerView.NO_POSITION && (layoutPos == 0 || hasHeader(
                adapterPos))) {
                ...
                c.restore();
                //保存绘制的header的区域
                mHeaderRects.put(adapterPos,
                    new Rect(left, top, left + header.getWidth(), top + header.getHeight()));
            }
        }
    }
    ...
    public boolean findHeaderClickView(View view, int x, int y) {
        ...
        for (int i = 0; i < mHeaderRects.size(); i++) {
            ...
            if (rect.contains(x, y)) {
                Rect vRect = new Rect();
                // 需要响应点击事件的区域在屏幕上的坐标
                vRect.set(rect.left + view.getLeft(), rect.top + view.getTop(),
            ...
            for (int i = 1; i < count; i++) {
                ...
                if (nextpos != RecyclerView.NO_POSITION) {
                    ...
                    if (currentId != nextId) {
                        ...
                        final int offset = ((int)next.getY()) - (headerHeight + getHeader(parent,
                            nextpos).getHeight());
