package com.comm.util.dialog.dw.progress;
...
public class ArcProgress extends View {
    ...
    private final int DEFAULTSIZE = (int)dp2px(getResources(), 200);
    ...
    protected void initByAttributes(TypedArray attributes) {
        //画弧度的颜色
        finishedStrokeColor = attributes.getColor(R.styleable.ArcProgress_arc_finished_color,
            default_finished_color);
        //底色
        unfinishedStrokeColor = attributes.getColor(R.styleable.ArcProgress_arc_unfinished_color,
            default_unfinished_color);

        //进度条字体颜色
        progressTextColor = attributes.getColor(R.styleable.ArcProgress_arc_progress_text_color,
            default_text_color);
        //进度条字体大小
        progressTextSize = attributes.getDimension(R.styleable.ArcProgress_arc_progress_text_size,
            default_text_size);

        //进度条上面字体颜色
        progressTextTopColor = attributes.getColor(
            R.styleable.ArcProgress_arc_progress_text_top_color, default_text_color);
        //进度条上面字体大小
        progressTextTopSize = attributes.getDimension(
            R.styleable.ArcProgress_arc_progress_text_top_size, default_text_size);
        //进度条上面
        progressTextTop = attributes.getString(R.styleable.ArcProgress_arc_progress_text_top_text);

        //进度条下面
        progressTextBottom = attributes.getString(
            R.styleable.ArcProgress_arc_progress_text_bottom_text);
        //进度条下面字体颜色
        progressTextBottomColor = attributes.getColor(
            R.styleable.ArcProgress_arc_progress_text_bottom_color, default_text_color);
        //进度条下面字体大小
        progressTextBottomSize = attributes.getDimension(
            R.styleable.ArcProgress_arc_progress_text_bottom_size, default_text_size);
        ...
        //旁边字体的颜色
        progressSuffixTextColor = attributes.getColor(
            R.styleable.ArcProgress_arc_progress_suffix_text_color, default_text_color);
        //旁边字体的宽度
        progressSuffixTextSize = attributes.getDimension(
            R.styleable.ArcProgress_arc_progress_suffix_text_size, default_text_size);
        ...
        //弧度的宽度
        strokeWidth = attributes.getDimension(R.styleable.ArcProgress_arc_stroke_width,
            default_stroke_width);
        //底部文字大小
        bottomTextSize = attributes.getDimension(R.styleable.ArcProgress_arc_bottom_text_size,
    ...
    public void startAnim(int duration) {
        if (mValueAnimator == null) {
            mValueAnimator = ValueAnimator.ofInt(0, (int)getProgress());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        rectF.set(strokeWidth / 2f, strokeWidth / 2f, widthSize - strokeWidth / 2f,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //        else
        String text = null;
        if ((decimal_digits) == 0) {text = Math.round(getProgress()) + progressSuffixText;} else {
        ...
        if (!TextUtils.isEmpty(text)) {
            ...
            if (isText) {
                canvas.drawText(mText, (getWidth() - textPaint.measureText(mText)) / 2.0f,
            ...
            if (!TextUtils.isEmpty(getProgressTextTop())) {
                ...
                canvas.drawText(getProgressTextTop(),
                    (getWidth() - textPaint.measureText(getProgressTextTop())) / 2.0f,
                    textBaseline + textHeight - dp2px(getResources(), 20), textPaint);
            }
            if (!TextUtils.isEmpty(getProgressTextBottom())) {
                ...
                canvas.drawText(getProgressTextBottom(),
                    (getWidth() - textPaint.measureText(getProgressTextBottom())) / 2.0f,
                    textBaseline - textHeight / 2 + dp2px(getResources(), 15), textPaint);

            }

            //            textPaint.setTextSize(suffixTextSize);
            //            float suffixHeight = textPaint.descent() + textPaint.ascent();
            //            canvas.drawText(suffixText, getWidth() / 2.0f + textPaint.measureText
            //            (text) + suffixTextPadding, textBaseline + textHeight - suffixHeight,
            //            textPaint);
        }
        ...
        if (!TextUtils.isEmpty(getBottomText())) {
            ...
            float textHeight = textPaint.descent() + textPaint.ascent();
            float textBaseline = (getHeight() - textHeight) / 2.0f;
            canvas.drawText(getBottomText(),
                (getWidth() - textPaint.measureText(getBottomText())) / 2.0f, bottomTextBaseline,
                textPaint);
        }
    }
