...
public class RxJavaRenActivity extends AppCompatActivity {
    ...
    public void doOnNext() {
        ...
        configMap.put("env", "ENV_CS");
        Observable<BaseCount<List<ConfigEnv>>> envApi = RetrofitFactory.configRetrofitService(
            IApiStores.class).getUnitList();
        RetrofitFactory.configRetrofitService(IApiStores.class).getAppConfig(
                GsonUtil.loginnoDoctorIdBody(configMap))
            ...
            .flatMap(
                new Function<BaseCount<SigleField>, ObservableSource<BaseCount<List<ConfigEnv>>>>() {
                    @Override
                    public ObservableSource<BaseCount<List<ConfigEnv>>> apply(
                        BaseCount<SigleField> sigleFieldBaseCount) throws Exception {
    ...
    public void cacheUrl() {
        RetrofitFactory.configRetrofitService(IApiStores.class).getAppConfigSJ(
                GsonUtil.loginnoDoctorIdBody(configMap))
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe(new Consumer<Response<BaseCount<SigleField>>>() {
                @Override
                public void accept(Response<BaseCount<SigleField>> baseCountResponse)
                    throws Exception {
            ...
            .flatMap(
                new Function<BaseCount<SigleField>, ObservableSource<BaseCount<List<ConfigEnv>>>>() {
                    ...
                    public ObservableSource<BaseCount<List<ConfigEnv>>> apply(
                        BaseCount<SigleField> sigleFieldBaseCount) throws Exception {
                        if (sigleFieldBaseCount.getMeta().getStatusCode() == 0) {
                            //                            Timber.i(sigleFieldBaseCount.getData()
                            //                            .getConfig());
                            return Observable.error(new Exception());
                        }
                        return envApi;
